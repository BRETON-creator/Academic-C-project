...
/**
 * @brief Fonction qui donne en sortie le plus petit rectangle contenant une liste de rectangle en entrée. Permet
 * d'optimiser le nombre de rectangles à mettre à jour dans ei_app_run.
 *
...
/**
 * Fonction modifiant un ensemble de points "circle" afin de créer une frame arrondie aux bords.
 * @param circle
 * @param rect
 * @param radius
 */
void give_rounded_frame(ei_point_t* circle, ei_rect_t rect, int radius) {
        ...
        for (int i = 0; i < 40; i++){
                ...
                if (0 <= i && i < 10) center = (ei_point_t) {(rect.top_left.x) + (rect.size.width) - radius,
                    (rect.top_left.y) + radius};
                if (10 <= i && i < 20) center = (ei_point_t) {(rect.top_left.x) + radius,
                    (rect.top_left.y) + radius};
                if (20 <= i && i < 30) center = (ei_point_t) {(rect.top_left.x) + radius,
...
/**
 *Fonction modifiant un ensemble de points "lower_frame" afin de créer une frame dont les bords de la partie basse
 *sont arrondies.
 * @param rounded_frame
 * @param rect
 * @param h
...
/**
 *Fonction modifiant un ensemble de points "upper_frame" afin de créer une frame dont les bords de la partie haute
 *sont arrondies.
 * @param rounded_frame
 * @param rect
 * @param h
...
/**
 * Fournit un point permettant de placer un rectangle de texte en fonction de la taille size_text et de l'ancrage du
 * texte.
 * @param rect
 * @param anchor
 * @param size_text
